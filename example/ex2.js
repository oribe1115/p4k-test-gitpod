...
function game () {
  ...
  if (enemy.alive) {
    ...
    // 弾と衝突したとき
    if (enemy.x < shot.x + shot.w && shot.x < enemy.x + enemy.w) {
      if (enemy.y < shot.y + shot.h && shot.y < enemy.y + enemy.h) {
        // 被弾音を鳴らす
        SE['Hit'].play()
  ...
  // 敵の弾と接触したとき
  if (player.x < enemyShot.x + enemyShot.w &&
    enemyShot.x < player.x + player.w) {
    if (player.y < enemyShot.y + enemyShot.h &&
      enemyShot.y < player.y + player.h) {
      // シーンをゲームオーバーにする
      scene = 'GAMEOVER'
    }
  }
